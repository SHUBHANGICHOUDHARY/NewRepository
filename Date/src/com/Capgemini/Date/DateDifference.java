...
public class DateDifference 
{
	...
	private static long remainingDaysInMonth(MyDate date){
		...
		remainingDays= daysInMonth[month-1]-date.getDate();
	...
	private static long remainingDaysInYear(MyDate startDate) {
		...
		for(int month = startDate.getMonth()+1; month <= 12; month++)
		{
			daysRemaining = daysRemaining + daysInMonth[month-1];
		}
		
		daysRemaining = daysRemaining + (daysInMonth[startDate.getMonth()-1]-startDate.getDate());
		
		if(isLeapYear(startDate) && startDate.getMonth()>=FEB)
			daysRemaining++;
	...
	private static long leadingYear(MyDate endDate) {
		...
		for(int month = 1; month < endDate.getMonth(); month++)
		{
			daysRemaining = daysRemaining + daysInMonth[month-1];
		}
		
		daysRemaining = daysRemaining + endDate.getDate();
		
		if(isLeapYear(endDate) && endDate.getMonth()>=FEB)
			daysRemaining++;
